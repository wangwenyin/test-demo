            ...
            <table class="table-report">
              <tr class="tr">
                ...
                <td colspan="3" class="td" >
                  <el-select v-model="form.appraiser" :multiple-limit="2" multiple placeholder="请选择" style="width:100%">
...
export default {
  ...
  methods: {
    ...
    getOption() {
      ...
      // 初始化注册估价师列表 （注意把否改成是）
      const param = { isCertified: '是', limit: 100, page: 1 }
    ...
    onPreviousStepClick() {
      gotoback(this.taskId).then(response => {
        if (response.code === 200) {
          ...
          } else if (response.status === '成本测算') {
            this.$router.push({ name: 'SingleappAppraisedCost', query: { taskId: this.taskId }})
          } else {
