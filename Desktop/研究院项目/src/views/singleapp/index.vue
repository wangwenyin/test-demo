<template>
  <div>
    <el-row class="search-container" style="padding: 5px">
      ...
      <el-col :span="8">
        <el-input v-model="searchOption.text" placeholder="请输入查询内容" size="small" @keyup.enter.native="onInputEnter">
    ...
    <el-row style="padding:5px">
      <el-table ref="taskTable" :data="taskList" :header-cell-style="changeHeaderStyle" style="width:100%" border fit stripe size="small" >
        ...
        <el-table-column label="操作" width="150" align="center">
          <template slot-scope="scope">
            <router-link :to="{ name:'SingleTaskDetail' , query: { taskId: scope.row.id,status:scope.row.status,method: scope.row.method }}">
  ...
  methods: {
    ...
    getTaskList(val) {
      ...
      const params = {
        ...
        name: (this.searchOption.clause === '任务名称' ? (this.searchOption.text === '' ? null : this.searchOption.text) : null),
        members: (this.searchOption.clause === '参与人员' ? (this.searchOption.text === '' ? null : this.searchOption.text) : null),
        usage: (this.searchOption.clause === '物业用途' ? (this.searchOption.text === '' ? null : this.searchOption.text) : null)
      }
      // console.log(params)
      if (!params.members) {
