<template>
  <div>
    ...
    <el-form v-if="form.result" ref="form" label-width="100px" label-position="right">
        ...
        <el-table ref="taskTable" :data="form.app_level" :header-cell-style="changeHeaderStyle" style="width:100%" border fit stripe size="small" >
          ...
          <el-table-column label="案例详情" width="100">
            <template slot-scope="scope">
              <div>
                <el-button size="mini" type="text" @click="onViewScopeClick(scope.row)">查看</el-button>
              </div>
            </template>
          </el-table-column>
        </el-table>
      ...
      <el-row>
        <span class="row-text">{{ form.result.level }}</span>
      </el-row>
      <el-row >
...
export default {
  ...
  methods: {
    onViewScopeClick(val) {
      this.baseinfo.scope = val.type
      const routerData = {
        name: 'CaseSearch',
