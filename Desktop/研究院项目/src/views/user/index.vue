<template>
  <div>
    <el-row class="search-box" >
      ...
      <el-input v-model="searchOption.text" placeholder="请输入内容" class="input-with-select" style="width:500px;float:right" size="small" @keyup.enter.native="onInputEnter">
...
export default {
  ...
  methods: {
    ...
    beforeRemovedFile(file) {
      deleteImage(this.editForm.login_name).then(res => {
        if (res.code === 200) {
    ...
    onBtnEditUserClick(index, row) {
      const url = 'http://test.daoheit.com'
      // 根据row信息生成对象
      this.editForm = {
        title: '编辑用户',
        ...
        cert_image_path: `${url}${row.cert_image_path}`
      }
