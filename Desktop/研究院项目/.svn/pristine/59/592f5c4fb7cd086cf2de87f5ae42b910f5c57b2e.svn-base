<template>
  <div>
    <div v-if="appObjectList.length < 2">
      <el-steps :active="step" align-center>
        <el-step :description="editors + ' '+ time" title="创建任务" @click.native="onCreatTaskClick(step)" />
        <el-step :description="editors + ' '+ time1" title="对象选择" @click.native="onObjSelectClick(step)" />
        <el-step :description="editors1 + ' '+ time1" title="对象描述" @click.native="onObjDescribeClick(step)" />
        <el-step :description="editors1 + ' '+ time2" title="价格评估" @click.native="onAppraisedPriceClick(step)" />
        <el-step :description="editors + ' '+ time2" title="报告撰写" @click.native="onReportClick(step)" />
        <el-step title="任务完成" />
      </el-steps>
    </div>
    <div v-else>
      <el-steps :active="step" align-center>
        ...
        <el-step :description="editors + ' '+ time1" title="对象选择" @click.native="onObjSelectClick(step)" />
        <el-step :description="editors1 + ' '+ time1" title="对象描述" @click.native="onObjDescribeClick(step)" />
        <el-step :description="editors1 + ' '+ time2" title="设置基准单元" @click.native="onSetDatumClick(step)" />
        <el-step :description="editors + ''+ time2" title="评估基准单元" @click.native="onAppraisedPriceClick(step)" />
        <el-step :description="editors + ''+ time2" title="评估全部对象" @click.native="onAppraisedAllClick(step)" />
        <el-step :description="editors " title="报告撰写" @click.native="onReportClick(step)" />
        <el-step title="任务完成" />
      </el-steps>
    </div>
    <el-dialog :visible.sync="dialogSelectFun" title="选择估价方法" >
      <el-select v-model="gjff" class="search" size="small" placeholder="估价方法">
        ...
        <el-option label="假设开发法" value="假设开发法" />
      </el-select>
    ...
    <el-dialog :visible.sync="dialog" title="请选择要进行的操作" >
    ...
    <el-dialog :visible.sync="dialogSetDatumUnit" title="设置基准单元" >
      <SetDatumUnit :evaluation-object="appObjectList" :datum-type="task.datumType" :datum-id="task.datumId" @SetDatum="SetDatum" />
      <div slot="footer" style="margin-top:-15px">
